...
export default async function CustomerReschedulePaymentPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <main className="space-y-8 pb-16">
      <SectionHeading eyebrow="Rescheduling payment" title="Complete the additional amount" description="Your original confirmed reservation remains valid until this additional payment is verified." />
      ...
      <section className="rounded-2xl border border-white/10 bg-stone-950/50 p-5 text-sm leading-7 text-stone-300"><p>Payment reference: <span className="font-medium text-white">{reschedule.additionalPayment.providerReference}</span></p><p>Include this reference in the transfer remarks when possible.</p>{reschedule.customerNote ? <p className="mt-2 text-amber-100">{reschedule.customerNote}</p> : null}</section>
