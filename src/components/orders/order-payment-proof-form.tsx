...
export function OrderPaymentProofForm({ bookingOrderId }: { bookingOrderId: string }) {
  ...
  useEffect(() => {
    if (state.error?.includes("booking order has expired")) {
      router.refresh();
    }
  }, [router, state.error]);
  ...
  return (
    <form action={action} className="space-y-5 rounded-[1.75rem] border border-white/10 bg-white/5 p-6" onSubmit={(event) => {
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <label className="space-y-2 text-sm text-stone-200"><span>Transfer reference number</span><input className="h-11 w-full rounded-2xl border border-white/10 bg-stone-900/80 px-4 text-base text-white" name="externalReference" required />{state.fieldErrors?.externalReference ? <p className="text-rose-300">{state.fieldErrors.externalReference}</p> : null}</label>
        <label className="space-y-2 text-sm text-stone-200 md:col-span-2"><span>Receipt screenshot or image</span><input accept="image/jpeg,image/png,image/webp,image/gif" className="w-full rounded-2xl border border-white/10 bg-stone-900/80 px-4 py-3 text-base text-white" name="proofImage" onChange={(event) => { const file = event.currentTarget.files?.[0]; setFileError(file && file.size > maxProofFileSizeBytes ? "Payment proof image must be 5MB or smaller." : null); }} required type="file" /><p className="text-xs text-stone-400">JPEG, PNG, WebP, or GIF up to 5MB.</p>{fileError || state.fieldErrors?.proofImage ? <p className="text-rose-300">{fileError ?? state.fieldErrors?.proofImage}</p> : null}</label>
      </div>
