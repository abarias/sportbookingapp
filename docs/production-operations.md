...
Run `npm run test:e2e` with a seeded local database and the app running locally. The Playwright smoke suite covers public facility browsing, single-booking checkout, customer proof submission, customer-to-admin payment verification and rejection recovery, cancellation and slot release, payment-proof remediation after staff requests additional evidence, administrative same-price/lower-price/higher-price rescheduling and original-slot or adjustment handling, cart checkout and consolidated payment proof, concurrent conflicting cart checkout with one successful order and one rejected checkout, expired consolidated-order recovery and slot release, a completed new-customer cash walk-in booking, weak-password registration preservation, registration and email verification using a unique `delivered+uat-<timestamp>@resend.dev` test recipient, customer/admin route denial, and least-privilege Receptionist, Booking Admin, and Social Media role journeys. Use synthetic seeded accounts only. Local and CI seeds also create `receptionist@sportbooking.local`, `booking-admin@sportbooking.local`, and `social-media@sportbooking.local` with their corresponding seeded roles; these accounts are never created by a non-local seed. If port 3000 is occupied, set `E2E_BASE_URL` to an already-running local server or run with `E2E_PORT=3001` after stopping other Next development servers for this repository.
