...
Run `npm run test:e2e` with a seeded local database and the app running locally. The Playwright smoke suite covers public facility browsing, customer booking checkout, adding a schedule to the cart, consolidated checkout and proof submission, a customer-to-admin consolidated payment verification handoff, a completed new-customer cash walk-in booking, the customer booking timeline, a representative 390px mobile booking layout, admin payment-queue/walk-in workspace access, and customer denial of admin access. Use synthetic seeded accounts only. If port 3000 is occupied, run with `E2E_PORT=3001`.
